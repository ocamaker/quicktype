...
import { GraphQLInput } from "@ocamaker/quicktype-graphql-input";
import { schemaForTypeScriptSources } from "@ocamaker/quicktype-typescript-input";

import { CompressedJSONFromStream } from "./CompressedJSONFromStream";
import { introspectServer } from "./GraphQLIntrospection";
import type { GraphQLTypeSource, JSONTypeSource, SchemaTypeSource, TypeSource } from "./TypeSource";
...
async function sourceFromFileOrUrlArray(name: string, filesOrUrls: string[], httpHeaders?: string[]): Promise<JSONTypeSource> {
  ...
  return [
    {
      header: "Synopsis",
      content: [
        `$ quicktype [${chalk.bold("--lang")} LANG] [${chalk.bold("--src-lang")} SRC_LANG] [${chalk.bold("--out")} FILE] FILE|URL ...`,
        "",
...
function parseOptions(definitions: OptionDefinition[], argv: string[], partial: boolean): Partial<CLIOptions> {
  ...
  for (const optionDefinition of definitions) {
    ...
    if (optionDefinition.kind !== "cli") {
      (options.rendererOptions as Record<typeof optionDefinition.name, unknown>)[optionDefinition.name] = optionValue;
    } else {
...
async function getSourceURIs(options: CLIOptions): Promise<Array<[string, string[]]>> {
  if (options.srcUrls !== undefined) {
    const json = parseJSON(await readFromFileOrURL(options.srcUrls, options.httpHeader), "URL grammar", options.srcUrls);
...
export function jsonInputForTargetLanguage(targetLanguage: string | TargetLanguage, languages?: TargetLanguage[], handleJSONRefs = false): JSONInput<Readable> {
...
async function makeInputData(sources: TypeSource[], targetLanguage: TargetLanguage, additionalSchemaAddresses: readonly string[], handleJSONRefs: boolean, httpHeaders?: string[]): Promise<InputData> {
...
function stringSourceDataToStreamSourceData(src: JSONSourceData<string>): JSONSourceData<Readable> {
  return {
    ...
    samples: src.samples.map((sample) => stringToStream(sample) as unknown as Readable),
  };
}
  ...
  switch (options.srcLang) {
    case "graphql":
      ...
      if (options.graphqlIntrospect !== undefined) {
        schemaString = await introspectServer(options.graphqlIntrospect, withDefault(options.httpMethod, "POST"), withDefault<string[]>(options.httpHeader, []));
    ...
    case "postman":
      for (const collectionFile of options.src) {
        ...
        const { sources: postmanSources, description } = sourcesFromPostmanCollection(collectionJSON, collectionFile);
        for (const src of postmanSources) {
          sources.push(Object.assign({ kind: "json" }, stringSourceDataToStreamSourceData(src)) as JSONTypeSource);
        }
...
export function writeOutput(cliOptions: CLIOptions, resultsByFilename: ReadonlyMap<string, SerializedRenderResult>): void {
  let onFirst = true;
