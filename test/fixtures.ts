...
function runEnvForLanguage(additionalRendererOptions: RendererOptions): NodeJS.ProcessEnv {
  ...
  for (const option of Object.keys(additionalRendererOptions)) {
    newEnv["QUICKTYPE_" + option.toUpperCase().replace("-", "_")] = (additionalRendererOptions[option as keyof typeof additionalRendererOptions] as Option<string, unknown>).name;
  }
  return newEnv;
}

function comparisonArgs(language: languages.Language, inputFilename: string, expectedFilename: string, additionalRendererOptions: RendererOptions): ComparisonArgs {
  return {
...
export abstract class Fixture {
  ...
  runMessageStart(sample: Sample, index: number, total: number, cwd: string, shouldSkip: boolean): string {
    ...
    const messageParts = ["*", chalk.dim(`[${index + 1}/${total}]`), chalk.magenta(this.name) + chalk.dim(`(${rendererOptions})`), path.join(cwd, chalk.cyan(path.basename(sample.path)))];
    if (shouldSkip) {
...
abstract class LanguageFixture extends Fixture {
  ...
  abstract test(filename: string, additionalRendererOptions: RendererOptions, additionalFiles: string[]): Promise<number>;
  ...
  async runWithSample(sample: Sample, index: number, total: number) {
    ...
    await inDir(cwd, async () => {
      ...
      try {
        numFiles = await timeout(this.test(sampleFile, sample.additionalRendererOptions, additionalFiles), MAX_TEST_RUNTIME_MS);
      } catch (e) {
    ...
    if (sample.saveOutput && OUTPUT_DIR !== undefined && this.language.output.indexOf("/") < 0) {
      const outputDir = path.join(OUTPUT_DIR, this.language.name, path.dirname(sample.path), path.basename(sample.path, path.extname(sample.path)));
      mkdirs(outputDir);
...
class JSONFixture extends LanguageFixture {
  ...
  async test(filename: string, additionalRendererOptions: RendererOptions, _additionalFiles: string[]): Promise<number> {
  ...
  getSamples(sources: string[]): { priority: Sample[]; others: Sample[] } {
    ...
    const prioritySamples = _.concat(testsInDir("test/inputs/json/priority", "json"), testsInDir("test/inputs/json/samples", "json"));
    ...
    const combinationInputs = _.map([1, 2, 3, 4], (n) => _.find(prioritySamples, (p) => p.endsWith(`/priority/combinations${n}.json`)));
      ...
      const quickTestSamples = _.chain(this.language.quickTestRendererOptions)
        .flatMap((qt) => {
          if (Array.isArray(qt)) {
            ...
            const input = _.find(([] as string[]).concat(prioritySamples, miscSamples), (p) => p.endsWith(`/${filename}`));
...
class JSONSchemaJSONFixture extends JSONToXToYFixture {
  ...
  async test(filename: string, additionalRendererOptions: RendererOptions, additionalFiles: string[]): Promise<number> {
...
const skipTypeScriptTests = [
  ...
  "3536b.json",
  "3e9a3.json", // duplicate top-level type: https://github.com/ocamaker/quicktype/issues/726
...
class JSONSchemaFixture extends LanguageFixture {
  ...
  async test(_sample: string, additionalRendererOptions: RendererOptions, additionalFiles: string[]): Promise<number> {
    if (this.language.compileCommand) {
      await execAsync(this.language.compileCommand);
    }
    ...
    for (const filename of additionalFiles) {
      if (failExtensions.some((ext) => filename.endsWith(ext))) {
        callAndExpectFailure(`Expected failure on input ${filename}`, () => exec(defined(this.language.runCommand)(filename), runEnvForLanguage(additionalRendererOptions), false).stdout);
      } else {
...
class GraphQLFixture extends LanguageFixture {
  ...
  async runQuicktype(filename: string, additionalRendererOptions: RendererOptions): Promise<void> {
    ...
    await quicktypeForLanguage(this.language, filename, "graphql", false, additionalRendererOptions, schemaFilename);
  }
  ...
  async test(_filename: string, additionalRendererOptions: RendererOptions, additionalFiles: string[]): Promise<number> {
    if (this.language.compileCommand) {
      await execAsync(this.language.compileCommand);
    }
    if (this.language.runCommand === undefined) return 0;
...
class CommandSuccessfulLanguageFixture extends LanguageFixture {
  ...
  async test(filename: string, _additionalRendererOptions: RendererOptions, _additionalFiles: string[]): Promise<number> {
    if (this.language.compileCommand) {
      await execAsync(this.language.compileCommand);
    }
