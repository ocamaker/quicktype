...
import { ArrayType, type ClassProperty, type ClassType, EnumType, MapType, type Type, type TypeKind, type UnionType } from "../../Type";
    ...
    this.emitBlockWithAccess([structOrClass, " ", className, this.getProtocolString(structOrClass)], () => {
      if (this._options.dense) {
        ...
        this.forEachClassProperty(c, "none", (name, jsonName, p) => {
          ...
          if ((lastProperty && !p.equals(lastProperty)) || lastNames.length >= MAX_SAMELINE_PROPERTIES || description !== undefined) {
            emitLastProperty();
          }
      ...
      if (
        ...
        // https://github.com/ocamaker/quicktype/issues/899
...
encoder.dateEncodingStrategy = .formatted(formatter)`);
  ...
  private readonly emitSupportFunctions4 = (): void => {
    ...
    if (this._options.convenienceInitializers) {
      this.ensureBlankLine();
      this.forEachTopLevel("leading-and-interposing", (t: Type, name: Name) => this.emitTopLevelMapAndArrayConvenienceInitializerExtensions(t, name));
    }
...
fileprivate func responseDecodable<T: Decodable>(queue: DispatchQueue? = nil, completionHandler: @escaping (DataResponse<T>) -> Void) -> Self {
	return response(queue: queue, responseSerializer: decodableResponseSerializer(), completionHandler: completionHandler)
      ...
      this.forEachTopLevel("leading-and-interposing", (_, name) => {
        ...
        this.emitBlock(["func response", name, "(queue: DispatchQueue? = nil, completionHandler: @escaping (DataResponse<", name, ">) -> Void) -> Self"], () => {
