...
import { capitalizeFirstLetter, escapeDoubleQuotes, escapeNewLines, memberNameStyle, simpleNameStyle, stringEscape } from "./utils";
...
export class ElixirRenderer extends ConvenienceRenderer {
  ...
  protected forbiddenForObjectProperties(_c: ClassType, _classNamed: Name): ForbiddenWordsInfo {
    return {
      names: reservedWords as unknown as string[],
  ...
  private elixirType(t: Type, isOptional = false): Sourcelike {
    ...
    return matchType<Sourcelike>(
      ...
      (unionType) => {
        ...
        return [children.flatMap((element, index) => (index === children.length - 1 ? element : [element, " | "])), optional];
      },
    );
  }
  ...
  private patternMatchClauseDecode(t: Type, attributeName: Sourcelike, suffix = ""): Sourcelike {
    return matchType<Sourcelike>(
      ...
      (classType) => {
        ...
        return ["def decode_", attributeName, suffix, "(%{", requiredAttributeArgs, "} = value), do: ", this.nameForNamedTypeWithNamespace(classType), ".from_map(value)"];
      },
      (_mapType) => ["def decode_", attributeName, suffix, "(value) when is_map(value), do: value"],
      (enumType) => ["def decode_", attributeName, suffix, "(value) when is_binary(value)", ", do: ", this.nameForNamedTypeWithNamespace(enumType), ".decode(value)"],
      (_unionType) => [],
    );
  }
  ...
  private patternMatchClauseEncode(t: Type, attributeName: Sourcelike, suffix = ""): Sourcelike {
    return matchType<Sourcelike>(
      ...
      (classType) => {
        ...
        return ["def encode_", attributeName, suffix, "(%", this.nameForNamedTypeWithNamespace(classType), "{} = value), do: ", this.nameForNamedTypeWithNamespace(classType), ".to_map(value)"];
      },
      (_mapType) => ["def encode_", attributeName, suffix, "(value) when is_map(value), do: value"],
      (enumType) => ["def encode_", attributeName, suffix, "(value) when is_atom(value)", ", do: ", this.nameForNamedTypeWithNamespace(enumType), ".encode(value)"],
      (_unionType) => [],
    );
  }
  ...
  protected emitPatternMatches(types: Type[], name: Sourcelike, parentName: Sourcelike, suffix = "", optional = false): void {
  ...
  private fromDynamic(t: Type, jsonName: string, name: Name, optional = false): Sourcelike {
    ...
    return matchType<Sourcelike>(
      ...
      (arrayType) => {
        ...
        if (optional) {
          return ["m", '["', jsonName, '"] && Enum.map(m["', jsonName, '"], &', this.nameOfTransformFunction(arrayElement, name, false, "_element"), "/1)"];
        }

        return ['Enum.map(m["', jsonName, '"], &', this.nameOfTransformFunction(arrayElement, name, false, "_element"), "/1)"];
      },
      (classType) => [optional ? [primitive, " && "] : "", this.nameForNamedTypeWithNamespace(classType), ".from_map(", primitive, ")"],
      (mapType) => {
        ...
        if (mapType.values.kind === "union") {
          return ['m["', jsonName, '"]\n|> Map.new(fn {key, value} -> {key, ', this.nameOfTransformFunction(mapType.values, jsonName, false), "_value(value)} end)"];
        }
        if (mapType.values instanceof EnumType || mapType.values instanceof ClassType) {
          return ['m["', jsonName, '"]\n|> Map.new(fn {key, value} -> {key, ', this.nameOfTransformFunction(mapType.values, jsonName, false), "(value)} end)"];
        }

        return [primitive];
      },
      (enumType) => {
        return [optional ? [primitive, " && "] : "", this.nameOfTransformFunction(enumType, name), "(", primitive, ")"];
      },
      (unionType) => {
  ...
  private toDynamic(t: Type, e: Sourcelike, optional = false): Sourcelike {
    ...
    return matchType<Sourcelike>(
      ...
      (arrayType) => {
        ...
        if (optional) {
          return ["struct.", e, " && Enum.map(struct.", e, ", &", this.nameOfTransformFunction(arrayElement, e, true, "_element"), "/1)"];
        }

        return ["struct.", e, " && Enum.map(struct.", e, ", &", this.nameOfTransformFunction(arrayElement, e, true, "_element"), "/1)"];
      },
      (classType) => [optional ? ["struct.", e, " && "] : "", this.nameForNamedTypeWithNamespace(classType), ".to_map(", "struct.", e, ")"],
      (mapType) => {
        const mapValueTypes = [...mapType.values.getChildren()];
        const mapValueTypesNotPrimitive = mapValueTypes.filter((type) => !(type instanceof PrimitiveType));
        if (mapValueTypesNotPrimitive.length === 0) {
        ...
        if (mapType.values.kind === "union") {
          return ["struct.", e, "\n|> Map.new(fn {key, value} -> {key, ", this.nameOfTransformFunction(mapType.values, e, true), "_value(value)} end)"];
        }
        if (mapType.values instanceof EnumType || mapType.values instanceof ClassType) {
          return ["struct.", e, "\n|> Map.new(fn {key, value} -> {key, ", this.nameOfTransformFunction(mapType.values, e, true), "(value)} end)"];
        }

        return [expression];
      },
      (enumType) => {
        return [optional ? ["struct.", e, " && "] : "", this.nameForNamedTypeWithNamespace(enumType), ".encode(struct.", e, ")"];
      },
      (unionType) => {
        const unionTypes = [...unionType.getChildren()];
        const unionPrimitiveTypes = unionTypes.filter((type) => type.isPrimitive());
  ...
  protected emitModule(c: ClassType, moduleName: Name): void {
    this.emitBlock(["defmodule ", this.nameWithNamespace(moduleName), " do"], () => {
      ...
      this.forEachClassProperty(c, "none", (name, _jsonName, p) => {
        const last = --count === 0;
        const attributeRow = [[], ["  ", name, ": ", this.elixirType(p.type), p.isOptional ? " | nil" : "", last ? "" : ","]];
...
def to_json(data) do
  ...
  protected emitSourceStructure(): void {
    if (this.leadingComments !== undefined) {
      ...
      this.emitMultiline(`# This file was autogenerated using quicktype https://github.com/ocamaker/quicktype
#
