...
import { type TypeAttributes, combineTypeAttributes, emptyTypeAttributes, makeTypeAttributesInferred } from "../attributes/TypeAttributes";
...
import { type PrimitiveTypeKind, type TransformedStringTypeKind, isNumberTypeKind, transformedStringTypeTargetTypeKindsMap } from "../Type";
...
class Resolver {
  ...
  private async tryResolveVirtualRef(fetchBase: Location, lookupBase: Location, virtualRef: Ref): Promise<[JSONSchema | undefined, Location]> {
    let didAdd = false;
    ...
    for (;;) {
      ...
      const schema = canonical.addressURI === undefined ? undefined : await this._store.get(address, this._ctx.debugPrintSchemaResolving);
      if (schema === undefined) {
...
async function addTypesInSchema(resolver: Resolver, typeBuilder: TypeBuilder, references: ReadonlyMap<string, Ref>, attributeProducers: JSONSchemaAttributeProducer[]): Promise<void> {
  ...
  async function makeObject(
    ...
    const props = await mapMapSync(propertiesMap, async (propSchema, propName) => {
      ...
      const t = await toType(checkJSONSchema(propSchema, propLoc.canonicalRef), propLoc, makeNamesTypeAttributes(propName, true));
    ...
    if (additionalProperties === undefined || additionalProperties === true) {
      ...
      additionalPropertiesType = await toType(checkJSONSchema(additionalProperties, additionalLoc.canonicalRef), additionalLoc, singularizeTypeNames(attributes));
    }
  ...
  async function convertToType(schema: StringMap, loc: Location, typeAttributes: TypeAttributes): Promise<TypeRef> {
    ...
    function forEachProducedAttribute(cases: JSONSchema[] | undefined, f: (attributes: JSONSchemaAttributes) => void): void {
    ...
    function combineProducedAttributes(f: (attributes: JSONSchemaAttributes) => TypeAttributes | undefined): TypeAttributes {
    ...
    async function makeObjectType(): Promise<TypeRef> {
      ...
      // https://github.com/ocamaker/quicktype/issues/976
      if (additionalProperties === undefined && typeof schema.patternProperties === "object" && hasOwnProperty(schema.patternProperties, ".*")) {
    ...
    async function makeTypesFromCases(cases: unknown[], kind: string): Promise<TypeRef[]> {
      ...
      return await arrayMapSync(cases, async (t, index) => {
        ...
        return await toType(checkJSONSchema(t, caseLoc.canonicalRef), caseLoc, makeTypeAttributesInferred(typeAttributes));
      });
    }
...
function nameFromURI(uri: URI): string | undefined {
  ...
  return messageError("DriverCannotInferNameForSchema", {
    uri: uri.toString(),
  });
}

async function refsInSchemaForURI(resolver: Resolver, uri: URI, defaultName: string): Promise<ReadonlyMap<string, Ref> | [string, Ref]> {
  const fragment = uri.fragment();
  ...
  if (propertiesAreTypes) {
    if (typeof schema !== "object") {
      return messageError("SchemaCannotGetTypesFromBoolean", {
        ref: ref.toString(),
      });
    }
