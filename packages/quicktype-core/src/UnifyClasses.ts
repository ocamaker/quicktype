import { iterableFirst, setIntersect, setUnionInto } from "collection-utils";
...
function getCliqueProperties(clique: ObjectType[], builder: TypeBuilder, makePropertyType: (types: ReadonlySet<Type>) => TypeRef): [ReadonlyMap<string, ClassProperty>, TypeRef | undefined, boolean] {
  ...
  const properties = Array.from(propertyNames).map((name) => [name, new Set(), false] as [string, Set<Type>, boolean]);
...
/**
 * Checks if object types represent distinct alternatives (like oneOf) that should NOT be unified.
 * Returns true if the objects have disjoint required property sets, indicating they are
 * mutually exclusive alternatives rather than variations of the same type.
 */
function areDistinctAlternatives(objectTypes: ObjectType[]): boolean {
  if (objectTypes.length < 2) return false;

  // Get required (non-optional) properties for each object
  const requiredProps = objectTypes.map((o) => {
    const props = new Set<string>();
    for (const [name, prop] of o.getProperties()) {
      if (!prop.isOptional) {
        props.add(name);
      }
    }
  ...
  // Check if any two objects have disjoint required properties
  // This indicates they are likely distinct alternatives (oneOf)
  for (let i = 0; i < requiredProps.length; i++) {
    for (let j = i + 1; j < requiredProps.length; j++) {
      const propsI = requiredProps[i];
      const propsJ = requiredProps[j];

      // If both have required properties and they don't overlap,
      // these are distinct alternatives
      if (propsI.size > 0 && propsJ.size > 0) {
        const intersection = setIntersect(propsI, propsJ);
...
export class UnifyUnionBuilder extends UnionBuilder<BaseGraphRewriteBuilder, TypeRef[], TypeRef[]> {
  ...
  protected makeObject(objectRefs: TypeRef[], typeAttributes: TypeAttributes, forwardingRef: TypeRef | undefined): TypeRef {
    ...
    // If object types represent distinct alternatives (like oneOf), keep them as a union
    // instead of merging their properties together
    if (areDistinctAlternatives(objectTypes)) {
      const reconstitutedRefs = objectRefs.map((r) => this.typeBuilder.reconstituteTypeRef(r));
      return this.typeBuilder.getUnionType(typeAttributes, new Set(reconstitutedRefs), forwardingRef);
    }
    ...
    if (!this._makeObjectTypes && (hasNonAnyAdditionalProperties || (!hasProperties && hasAdditionalProperties))) {
      ...
      const additionalPropertyTypes = new Set(objectTypes.filter((o) => o.getAdditionalProperties() !== undefined).map((o) => defined(o.getAdditionalProperties()).typeRef));
